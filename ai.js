...
export async function chooseWithAI(candidates,recentTexts=[]){if(!Array.isArray(candidates)||!candidates.length)return null;if(!embedder||candidates.length<2)return candidates[Math.floor(Math.random()*candidates.length)]||candidates[0];const sample=shuffle(candidates).slice(0,32),recent=recentTexts.filter(Boolean).slice(-30);if(!recent.length)return{...sample[Math.floor(Math.random()*sample.length)],aiPick:true};try{const texts=sample.map(q=>`${q.q} ${Array.isArray(q.a)?q.a.join(' '):''}`);const vectors=await timeout(embedTexts([...texts,...recent]),PICK_TIMEOUT_MS,'AI picker timed out');const cv=vectors.slice(0,sample.length),rv=vectors.slice(sample.length);const scores=cv.map((vec,i)=>{const sims=rv.map(old=>dot(vec,old)),max=Math.max(...sims),avg=sims.reduce((a,b)=>a+b,0)/Math.max(1,sims.length);let peer=0,count=0;for(let j=0;j<cv.length;j++){if(j===i)continue;peer+=dot(vec,cv[j]);count++;}const novelty=(1-max)*.68+(1-avg)*.22+(1-(count?peer/count:0))*.10+Math.random()*.012;return{i,novelty,maxRecent:max};}).sort((a,b)=>b.novelty-a.novelty);const shortlist=scores.filter(x=>x.maxRecent<.88).slice(0,5),pool=shortlist.length?shortlist:scores.slice(0,Math.min(5,scores.length)),chosen=pool[Math.floor(Math.random()*pool.length)]||scores[0];return{...sample[chosen.i],aiPick:true,aiNovelty:Number(chosen.novelty.toFixed(3))};}catch(e){console.warn('AI question selection timed out/failed; using normal picker:',e);return sample[Math.floor(Math.random()*sample.length)]||candidates[0];}}
