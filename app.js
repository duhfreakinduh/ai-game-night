...
import { initAI, remixQuestion } from './ai.js';
...
function maybeEvent(){
  ...
  els.event.classList.add('hidden'); return 0;
}
...
function answerQuestion(index,button){
  ...
  player.attempts++; player.byCategory[currentQuestion.c] ||= {ok:0,n:0}; player.byCategory[currentQuestion.c].n++;
  $$('.answer-btn').forEach(b=>b.disabled=true);
  $(`.answer-btn[data-i="${currentQuestion.x}"]`)?.classList.add('correct');
  if(correct){
    button.classList.add('correct'); player.correct++; player.byCategory[currentQuestion.c].ok++; player.streak++;
    const streakBonus=Math.min(50,Math.max(0,(player.streak-1)*10));
    let pts=100+streakBonus+state.eventBonus; if(doubleActive) pts*=2; player.score+=pts;
    if(player.streak>=2) player.skill=Math.min(5,player.skill+0.18);
    els.feedback.textContent=`Correct! +${pts} points${player.streak>1?` • ${player.streak} streak!`:''}`; els.feedback.classList.add('good'); beep(720);
  }else{
    button.classList.add('wrong'); player.streak=0; player.skill=Math.max(1,player.skill-0.22);
    els.feedback.textContent=`Good try. ${currentQuestion.h}`; els.feedback.classList.add('bad'); beep(220);
  }
...
function useFifty(){
  const p=state?.players[state.turn]; if(!p||p.fifty<1) return; p.fifty--;
  const wrong=$$('.answer-btn').filter(b=>Number(b.dataset.i)!==currentQuestion.x); wrong.sort(()=>Math.random()-.5).slice(0,2).forEach(b=>{b.disabled=true;b.style.visibility='hidden'}); els.fiftyCount.textContent=p.fifty;
}
function useDouble(){const p=state?.players[state.turn];if(!p||p.double<1||doubleActive)return;p.double--;doubleActive=true;els.double.classList.add('active');els.doubleCount.textContent=p.double;}
function show(screen){[els.setup,els.game,els.score].forEach(x=>x.classList.remove('active'));screen.classList.add('active');window.scrollTo({top:0,behavior:'smooth'});}
function escapeHtml(s){return s.replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));}
function beep(freq){if(!soundOn)return;try{const c=new AudioContext(),o=c.createOscillator(),g=c.createGain();o.frequency.value=freq;g.gain.value=.04;o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+.08)}catch{}}

els.add.onclick=()=>addPlayer('',2); els.start.onclick=startGame; els.fifty.onclick=useFifty; els.double.onclick=useDouble; els.again.onclick=()=>show(els.setup);
els.sound.onclick=()=>{soundOn=!soundOn;els.sound.textContent=soundOn?'🔊':'🔇'};
els.ai.onchange=async()=>{if(els.ai.checked){const ok=await initAI(msg=>els.aiStatus.textContent=msg);els.ai.checked=ok;}else els.aiStatus.textContent='AI is off. Fast offline question bank is ready.';};
setupDefaults();
